...
export default function PagePlay() {
  ...
  const mergePlayListToFavorites = (video: any) => {
    const videoFavorite = favoriteList.find(
      (favorite: any) => favorite.id === video.id,
    )
  ...
  const handleFavorite = useCallback((status: boolean, data: any) => {
    if (status === true) {
      return addFavorite({ ...data, status })
    }
  ...
  const cardsMemo = useMemo(() => {
    return list.length
      ? list.map(mergePlayListToFavorites).map((data: any, index: number) => {
          return (
            <CardPlay
        ...
        <div className="page-play__container">
          ...
          <div className="page-play__videos">
            ...
            <div className="page-play__list">
              ...
              <button className="page-play__list-btn" onClick={getMoreVideos}>
                + videos
              </button>
            </div>
          </div>
